...
namespace BNF.Core.DecalSystem
{
    ...
    public sealed class DialogEditDecals : Window
    {
        ...
        public DialogEditDecals(Pawn pawn)
        {
            ...
            _profile = DecalUtil.ReadProfileFrom(pawn);
            _original = _profile;

            _symbols = DecalUtil.AllSymbols();
            _palette = DecalUtil.FixedPalette_LargeSpread();
        ...
        private void DrawHeader(Rect rect)
        {
            ...
            Widgets.Label(rect, "BNF_StyleDecalsTitle".Translate(_pawn.Name.ToStringShort));
        ...
        private void DrawCycler(Rect rect)
        {
            ...
            {
                _selectedIndex = (_selectedIndex - 1 + _symbols.Count) % _symbols.Count;
                SyncSelection();
            }

            if (Widgets.ButtonText(right, ">") && hasAny)
            {
                _selectedIndex = (_selectedIndex + 1) % _symbols.Count;
                SyncSelection();
            }
            ...
            Widgets.Label(label, _selectedSymbol != null ? _selectedSymbol.LabelCap : "BNF_Decals_NoSymbolsFound".Translate());
        ...
        private void DrawPreview(Rect rect)
        {
            ...
            Texture2D? tex = GetSymbolTex(_profile.SymbolPath);
            if (tex != null)
            {
                GUI.color = _profile.Active ? _profile.SymbolColor : new Color(1f, 1f, 1f, 0.25f);
        ...
        private void DrawPalette(Rect rect)
        {
            ...
            int count = Mathf.Min(_palette.Count, maxVisible);
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                Color col = _palette[i];

                Widgets.DrawBoxSolid(cr, col);
                Widgets.DrawBox(cr);

                if (_profile.Active && _profile.SymbolColor.IndistinguishableFrom(col))
        ...
        private void DrawColorButtons(Rect rect)
        {
            ...
            if (TryGetIdeoColor(_pawn, out Color ideoColor))
            {
        ...
        private void DrawInputs(Rect rect)
        {
            ...
            Widgets.DrawBoxSolid(swatch, _profile.SymbolColor);
            Widgets.DrawBox(swatch);
            TooltipHandler.TipRegion(swatch, DecalUtil.ColorToHex(_profile.SymbolColor));
            y = swatch.yMax + Tight;

            Widgets.Label(new Rect(rect.x, y, rect.width, labelH), "BNF_Decals_RGB".Translate());
            y += labelH + Tight;

            bool chR = DrawRGBRow(rect.x, y, rect.width, rowH, labelColW, "R", ref _rStr); y += rowH + rowGap;
            bool chG = DrawRGBRow(rect.x, y, rect.width, rowH, labelColW, "G", ref _gStr); y += rowH + rowGap;
            bool chB = DrawRGBRow(rect.x, y, rect.width, rowH, labelColW, "B", ref _bStr); y += rowH + Tight;

            if (chR || chG || chB) _rgbDirty = true;

            if (_rgbDirty && TryParseRGB(_rStr, _gStr, _bStr, out Color rgb))
            {
                SetColor(rgb);
                _hexStr = DecalUtil.ColorToHex(_profile.SymbolColor);
                _hexDirty = false;
            }
            ...
            string before = _hexStr;
            _hexStr = Widgets.TextField(field, before);
            if (_hexStr != before) _hexDirty = true;

            TooltipHandler.TipRegion(field, "RRGGBB");

            if (_hexDirty && DecalUtil.TryParseHexColor(_hexStr, out Color hc))
            {
                SetColor(hc);

                ToRGBInt(_profile.SymbolColor, out int rr, out int gg, out int bb);
                _rStr = rr.ToString();
                _gStr = gg.ToString();
                _bStr = bb.ToString();
                _rgbDirty = false;
        ...
        private void DrawApply(Rect rect)
        {
            ...
            {
                DecalUtil.ApplyAndRefresh(_pawn, _profile);
        ...
        private void SyncSelection()
        {
            ...
            if (_selectedSymbol == null)
            {
                ...
                {
                    int idx = (_selectedIndex + i) % _symbols.Count;
                    if (_symbols[idx] != null)
                    {
                        _selectedIndex = idx;
                        _selectedSymbol = _symbols[idx];
                        break;
                    }
                }
            }
        ...
        {
            if (_selectedSymbol == null) return "BNF_Decals_NoSelectionDesc".Translate();
            if (!_selectedSymbol.description.NullOrEmpty()) return _selectedSymbol.description;
